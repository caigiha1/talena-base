...
type FancyMultiSelectHiringProps = Readonly<{
...
export function FancyMultiSelectHiring({
  data,
  defaultSelected = [],
  onValueChange,
  className,
}: FancyMultiSelectHiringProps) {
  ...
  useEffect(() => {
    if (onValueChange) {
      onValueChange(selected.length ? selected.map((s) => s.id) : null);
    }
